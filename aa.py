...
# Ask for API ID, API hash
api_id = input(colored("Enter your Telegram API ID: ", "blue"))
api_hash = input(colored("Enter your Telegram API Hash: ", "blue"))

# Ask for number of sessions and create them dynamically
num_sessions = int(input(colored("Enter the number of sessions you want to provide: ", "blue")))

user_sessions = []

# Get session details from the user
for i in range(1, num_sessions + 1):
    user_session = input(colored(f"Enter session {i} file name or leave empty to skip: ", "blue")).strip()
    if user_session:  # Only add session if it is provided
        user_sessions.append(user_session)

# Ask for the delay between reports
delay_time = int(input(colored("Enter the delay between reports in seconds (e.g., 5): ", "blue")))
...
def main():
    ...
    async def run_session(user_session):
        ...
        client = TelegramClient(user_session, api_id, api_hash, session=MemorySession())
    ...
    # Run sessions asynchronously for multiple user sessions
    loop = asyncio.get_event_loop()
    tasks = [loop.create_task(run_session(user_session)) for user_session in user_sessions]
